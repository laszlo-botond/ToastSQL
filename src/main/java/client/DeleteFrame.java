...
public class DeleteFrame extends JFrame {
	...
	public DeleteFrame() {
		this.setTitle("ToastSQL Delete GUI");
...
//                System.out.println("Line: " + s);
    ...
    public void createTable() {
        // get fields
        activeFields = getPKsOfTable(cbDb.getSelectedItem().toString(), cbTable.getSelectedItem().toString());
    ...
    private String[] getPKsOfTable(String dbName, String tableName) {
        try {
        	...
        	// send message
        	String msg = "GETPKSOF " + dbName + " " + tableName;
        	msg = msg.length() + "\n" + msg;
    ...
    private void createAndSendRequest() {
        ...
        for (int i=0; i<tableRows; i++) {
            ...
            commandLine = "DELETE FROM " + cbTable.getSelectedItem() + "\nWHERE ";
            
            
            // add actual field values
            for (int j=0; j<tableCols; j++) {
                if (given[j]) {
                	commandLine = commandLine + activeFields[j] + " = " + table.getValueAt(i, j) + " AND ";
                }
                else {
                	setAnsLabel("Filter for whole PK!");
                	return;
                }
            }
            
            fullCommand = fullCommand + commandLine.substring(0, commandLine.length() - 4) + "\n";
        }
        System.out.println("DeleteFrame submit: " + fullCommand);
//        return;
